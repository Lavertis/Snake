...
def draw_egg(world):
    ...
    egg_size = world.snakeElementSize
...
def draw_snake_elements(world):
    for element in reversed(world.snakeElements):
        position_x = element.position.x * world.surfaceSize / world.mapSize + 2
        position_y = element.position.y * world.surfaceSize / world.mapSize + 2
        element_size = world.snakeElementSize
        pygame.draw.rect(world.screen, element.colour, (position_x, position_y, element_size, element_size))


def display_score_bar(world):
...
def display_score(world):
    text = 'HIGHSCORE: ' + str(round(world.highScore)) + ' ' * 18 + 'SCORE: ' + str(round(world.score))
